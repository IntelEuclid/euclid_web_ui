define(['jquery',
    ...
    'bootstrapSlider',
    'bootbox',
    'text!/templates/config/config-follower-template.html'

], function ($, _, Backbone, GlobalSettings, bootbox, configFollowerTemplate) {
    var configFollowerView = Backbone.View.extend({
        template: _.template(configFollowerTemplate),
        ros_node_name: "realsense_depth_follower",
        id_prefix: "realsense_depth_follower__follower_standalone_nodelet__",
        paramsClient: null,
        setParams: null,
        request: null,
        el: $("#body"),
        events: {
            'click button.follower_reset': 'reset',
            'click button.follower_save': 'save',
        },
        reset: function () {
        ...
        save: function () {
            this.eventAggregator.trigger("loading:on", { key: 'followersViewSave', msg: "Saving configurations..." });
            var self = this;
            var sliders = $("input.slider");
            $.each(sliders, function (i, s) {
                var key = $(s).data("key");
            ...
            self.eventAggregator.trigger("loading:off", { key: 'followersViewSave' });
        },
        loadRosParams: function () {
            var self = this;
            this.eventAggregator.trigger("loading:on", { key: 'followersViewLoad', msg: "Loading ROS Params..." });
            paramsClient.callService(request, function (result) {
                ...
                this.eventAggregator.trigger("loading:off", { key: 'followersViewLoad', msg: "Loading ROS Params..." });
            }, function (err) {
                this.eventAggregator.trigger("loading:off", { key: 'followersViewLoad', msg: "Loading ROS Params..." });
